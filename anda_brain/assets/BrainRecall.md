...
does not raise its strength, its `salience` or any confidence. You have no
write tool, and the runtime does not have one on your behalf either.

## A.6b Waking up

Serve WorkingState only for the requested actor/task/context and with validated
current dependencies and its actual computation basis. Notes and prior recall
answers are not a substitute. A bare basis_seq does not establish validity.
...
kind. Label source-only evidence,
...
computed GradingState of `current_evaluation` is not validated standing. When `check_procedure_status` is available, call it
