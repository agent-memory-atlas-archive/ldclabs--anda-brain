...
const plan = {
  types: ['AnswerStyle'], predicates: [], summary: 'stored',
  commands: [`MUTATE {
...
it('accepts the deprecated decay factor and writes no strength in settlement', async () => {
  ...
  const created = await post(e, space, 'execute_kip', {command: `CREATE CONCEPT ?p {
    TYPE "Insight" NAME "keep accessible" SET ATTRIBUTES { summary: "keep accessible" }
    SET FACET "MnemonicState" { memory_strength: 0.8 }
  }`})
  expect(created.status, await created.text()).toBe(200)
  const maintained = await post(e, space, 'maintenance', {parameters:{memory_strength_decay_factor:0.5}})
  expect(maintained.status, await maintained.text()).toBe(200)
  const read = await post(e, space, 'execute_kip_readonly', {command:'FIND(?c.facets["MnemonicState"].memory_strength) WHERE { ?c CONCEPT {type: "Insight"} } LIMIT 10'})
...
it('discovers every correction when one transaction exceeds a page', async () => {
  ...
  await brain.declareSymbols(['Tone'], [])
  ...
  const [seed] = await brain.executeKipBatch([{command: `MUTATE {
    ...
    CREATE CONCEPT ?preference {TYPE "Tone" NAME "Plain"}
    ${assertions.join('\n')}
